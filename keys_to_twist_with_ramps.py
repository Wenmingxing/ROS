...
def ramped_vel(v_prev,v_target,t_prev,t_now,ramp_rate):
	...
	if error<step: # we can get there within this timestep_we'redone
		return v_target
	else:
...
def ramped_twist(prev,target,t_prev,t_now,ramps):
	tw = Twist()
	tw.angular.z = ramped_vel(prev.angularz,target.angular.z,t_prev,t_now,ramps[0])
	tw.linear.x = ramped_vel(prev.linear.x,target.linear.x,t_prev,t_now,ramps[1])
	return tw

def send_twist():
	global g_last_twist_send_timw,g_target_twist,g_last_twist,g_vel_scales,g_vel_ramps,g_twist_pub
	t_now = rospy.Time.now()
	g_last_twist = ramped_twist(g_last_twist,g_target_twist,g_last_twist_send_time,t_now,g_vel,ramps)
...
def fetch_param(name,default):
	if rospy.has_param(name):
		...
		print("parameter [%s] noe defined. Defaulting to %.3f"%(name,defalut))
		return default
