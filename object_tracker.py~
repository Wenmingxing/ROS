...
class ObjectTracker():
	def __init__(self):
		...
		# How often shoule we update the robot's motion
		self.rate = rospy.get_param('~rate',10)
		r = rospy.Rate(self.rate)
		
		# The maximun rotarion speed in radians per second
		self.max_rotation_speed = rospy.get_param('~max_rotation_speed',2.0)
		
		# The minimunm rotation speed in radias per second
		...
		# The x threshold (% of image width) indicates how far off-centre the ROI needs to be in the x-derection before we react
		...
		# Get a lock the movement command
	...
	def set_cmd_vel(self,msg):
		...
		try:
			...
			if abs(percent_offser_x) > self.x_threshold:
...
if __name__ == '__main__':
	try:
		...
		rospy.loginfo('Object tracking node terminate.')	
